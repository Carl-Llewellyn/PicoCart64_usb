...
static void pi_read_raw(void *dest, uint32_t base, uint32_t offset,
                        uint32_t len) {
  assert(dest != NULL);
...
static void pi_write_raw(const void *src, uint32_t base, uint32_t offset,
                         uint32_t len) {
  assert(src != NULL);
  verify_memory_range(base, offset, len);
...
int main(void) {
  ...
  pi_read_raw(read_buf, PC64_CIBASE_ADDRESS_START, PC64_REGISTER_MAGIC,
              sizeof(uint32_t));
  ...
  data_cache_hit_writeback_invalidate(read_buf, sizeof(read_buf));
  pi_read_raw(read_buf, PC64_CIBASE_ADDRESS_START, PC64_REGISTER_FLASH_JEDEC_ID,
              sizeof(uint32_t));
  ...
  while (1) {
    ...
    if (fail_count > 0) {
      graphics_draw_box(disp, 0, 0, width, height,
                        graphics_make_color(x, 0, 0, 255));
      graphics_draw_box(disp, 0, count % (height - 10), width, 10,
                        graphics_make_color(127, 127, 127, 255));
    } else {
      graphics_draw_box(disp, 0, 0, width, height,
                        graphics_make_color(0, x, 0, 255));
      graphics_draw_box(disp, 0, count % 230, width, 10,
                        graphics_make_color(127, 127, 127, 255));
    }
    ...
    if (currt > delay) {
      pi_read_raw(read_buf, PC64_CIBASE_ADDRESS_START, PC64_REGISTER_UART_RX,
                  sizeof(uint32_t));
      printf("[ USB RX ] 0x%08lX.\n", read_buf32[0]);
      currt = 0;
    }
    currt++;
  }
