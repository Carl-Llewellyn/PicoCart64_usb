...
void n64_pi_run(void) {
  ...
  while (1) {
    ...
    if (last_addr >= CART_DOM1_ADDR2_START &&
        last_addr <= CART_DOM1_ADDR2_END) {
      ...
      do {
        ...
        uint32_t chunk_index =
            rom_mapping[(last_addr & 0xFFFFFF) >> COMPRESSION_SHIFT_AMOUNT];
      ...
      do {
        // Pre-fetch from the address
        next_word =
            pc64_uart_tx_buf[(last_addr & (sizeof(pc64_uart_tx_buf) - 1)) >> 1];
        ...
        if ((addr & 0xffff0000) == 0xffff0000) {
          ...
          last_addr += 2;
          next_word =
              pc64_uart_tx_buf[(last_addr & (sizeof(pc64_uart_tx_buf) - 1)) >>
                               1];
        } else {
          // New address
          break;
